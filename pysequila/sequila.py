...
def register(session: SparkSession):
    ...
    spark_session = (
        session._jvm.org.apache.spark.sql.SparkSession.builder()
        .enableHiveSupport()
        .getOrCreate()
    )
...
class SequilaSession(SparkSession):
    ...
    def __init__(self, session: SparkSession, jsparkSession=None):
        ...
        seq_session = session._jvm.org.apache.spark.sql.SequilaSession(
            session._jsparkSession
        )  # pylint: disable=W0212

        session._jvm.org.biodatageeks.sequila.utils.SequilaRegister.register(
            seq_session
        )
        session._jvm.org.biodatageeks.sequila.utils.UDFRegister.register(seq_session)
        ...
        if jsparkSession is None:
            if (
                ...
                and not self._jvm.SequilaSession.getDefaultSession()
                .get()
                .sparkContext()
                .isStopped()
            ):
        ...
        if (
            SequilaSession._instantiatedSession is None
